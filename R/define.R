...
define <- function(term,limitResult=5){
  ...
  if(exists(chartofind.a)){
    ...
    if(gfhrow){
      ...
      if(toupper(substr(chartofind,1,1))=="B") message("Due to size limitation, B words have been excluded in the first CRAN version. Only the github download currently contains the full dictionary library.")
      else message(paste0("No results found for ",chartofind," with this r2dictionary version."))
    }
